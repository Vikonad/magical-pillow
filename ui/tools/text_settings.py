...
class TextSettings(QWidget):
    def __init__(self):
        ...
        font.resize(400, 30)
        font_window = QPushButton("...")
        font_window.clicked.connect(self.open_font_picker)
        font_window.setFixedWidth(32)
        font_layout.addWidget(font)
        font_layout.addWidget(font_window)
        layout.addLayout(font_layout)

        self.button = QPushButton("Undock")
        self.button.clicked.connect(self.undock_tools_panel)
    ...
    def open_font_picker(self):
        self.font_picker = FontPreviewWindow()
        self.font_picker.show()
        self.font_picker.setWindowFlags(Qt.Tool | Qt.WindowStaysOnTopHint)
        self.font_picker.show()
        self.font_picker.raise_()
        self.font_picker.activateWindow()

    def undock_tools_panel(self):
        self.hide()
        if self.editor is None:
            # Pass self or main window as parent
            self.editor = TextSettingsWindow(parent=self)
        self.editor.show()
